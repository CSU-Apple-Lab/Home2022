...
export const useSelectHTMLElementById = <T extends HTMLElement>() =>
	curry((elementId: string): (() => Either<Error, T>) => {
		return () => {
			const target = <T>document.getElementById(elementId);
			switch (target) {
				case null:
					return left<Error, T>(
						new Error(`[selectHTMLElementById](id:${elementId})=>Specify element not found`)
					);
				default:
...
export const useMapHTMLElement = <T extends HTMLElement, Result>() =>
	curry((mapFn: (el: T) => Result, selectFn: () => Either<Error, T>): (() => Result) => {
		return ensure(() => map(mapFn)(selectFn()));
	})();

export const useAddEventListener = <T extends HTMLElement, K>() =>
	curry((selector: () => Either<Error, T>, k: keyof K, fn: () => void) => {
		ensure(selector)().addEventListener(<any>k, fn);
	});
export const useAddMediaEventListener = () =>
	useAddEventListener<HTMLMediaElement, HTMLMediaElementEventMap>();
...
export const addEventListenerById = pipe(
	useSelectHTMLElementById<HTMLMediaElement>(),
	useAddMediaEventListener()
);
