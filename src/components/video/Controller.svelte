...
<div
	use:whileHover
	transition:fly
	class={`z-50 fixed bottom-0 ${rolled ? 'w-52' : 'w-full lg:w-1/2'} transition-all duration-300`}
>
	<div
		class="w-full h-10 rounded-tr-2xl bottom-0 flex justify-between items-center bg-black shadow-black shadow-lg"
	>
		<div class="flex justify-start items-center">
			{#if isPaused}
				<PlayIcon
					role="button"
					class="w-7 h-7 m-5 opacity-30 hover:opacity-100 transition-opacity duration-300 text-white cursor-pointer"
					on:click={play}
				/>
			{:else}
				<PauseIcon
			...
			{#if minimize}
				<PlusIcon
					role="button"
					class="w-7 h-7 m-5 opacity-30 hover:opacity-100 transition-opacity duration-300 text-white cursor-pointer"
					on:click={() => setMinimize(false)}
				/>
			{:else}
				<MinusIcon
					role="button"
					class="w-7 h-7 m-5 opacity-30 hover:opacity-100 transition-opacity duration-300 text-white cursor-pointer"
					on:click={() => setMinimize(true)}
				/>
			{/if}
			{#if !rolled}
				<RewindIcon
		...
		{#if rolled}
			<ChevronDoubleRightIcon
				role="button"
				class="w-7 h-7 m-5 opacity-30 hover:opacity-100 transition-opacity duration-300 text-white cursor-pointer"
				on:click={() => setControls(false)}
			/>
		{:else}
			<ChevronDoubleLeftIcon
				role="button"
				class="w-7 h-7 m-5 opacity-30 hover:opacity-100 transition-opacity duration-300 text-white cursor-pointer"
				on:click={() => setControls(true)}
			/>
		{/if}
	</div>
</div>
